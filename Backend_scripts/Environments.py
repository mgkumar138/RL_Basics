...
# make environment
class LinearEnv:
    def __init__(self, nstates=4, nactions=2):
    ...
    def step(self, action):
        ...
        if action == 1:
            ...
            reward = 1
            if newstate < 0:
                newstate = 0 # dont let state go behind s0
                reward = -1 # setting reward to -1 compels agent to explore state
        if self.t >= self.maxt:
            reward = -1
            done = True
        self.state = newstate
...
class SimpleGrid:
    def __init__(self, x=5, y=4, rloc=4, stpos=0, obs=[2,7,12]):
        self.nstates = int(x * y)
        self.x = x
        self.y = y
        self.nactions = 4
        self.allstates = np.arange(self.nstates).reshape(y,x)
        self.rloc = rloc
        self.obs = obs
        self.maxt = 300
        self.stpos = stpos
        ...
        self.done = False
        self.t = 0
        self.topbound = np.arange(0,x)
        self.bottombound = np.arange(self.nstates-x,self.nstates)
        self.leftbound = np.arange(0,self.nstates,x)
        self.rightbound = np.arange(y,self.nstates, x)

    def reset(self):
        self.state = self.stpos
        self.reward = 0
        self.done = False
        self.t = 0
        return 0,0,False
    ...
    def step(self, action):
        ...
        newstate = self.state
        if action == 0: # UP
            if self.state not in self.topbound:
                newstate = self.state - self.x
        elif action == 1: # Right
            if self.state not in self.rightbound:
                newstate = self.state + 1
        elif action == 2: # down
            if self.state not in self.bottombound:
                newstate = self.state + self.x
        elif action == 3: # left
            if self.state not in self.leftbound:
                newstate = self.state - 1

        if newstate in self.obs: # if agent goes into obs, bring agent back
            newstate = self.state

        if newstate == self.rloc: # if agent reaches reward, disbruse reward
            self.reward = 1
            self.done = True
        ...
        self.state = newstate
        return newstate, self.reward, self.done
